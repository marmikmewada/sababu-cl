...
function AppNav() {
  ...
  const logout = useStore((state) => state.logout); // Get the logout function from Zustand store
    ...
    <nav className={`${'navbar sticky-top navbar-expand-lg navbar-dark'} ${styles.nav}`}>
      ...
      <div ref={menuRef} className={`collapse navbar-collapse ${isMenuOpen ? 'show' : ''}`}>
        <ul className="navbar-nav ms-auto">
          <List closeMenu={closeMenu} isAuthenticated={isAuthenticated} role={role} handleSignOut={handleSignOut} />
        </ul>
      </div>
    </nav>
  );
...
function List({ closeMenu, isAuthenticated, role, handleSignOut }) {
  return (
    <>
      ...
      {isAuthenticated && (
        <>
          ...
          <li className="nav-item">
            <NavLink className="nav-link" to="/" onClick={() => { closeMenu(); handleSignOut(); }}>
              Sign Out
            </NavLink>
          </li>
        </>
